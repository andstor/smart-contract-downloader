...
class ContractsDownloadManager:
    ...
    def download(self):
        ...
        with open(self.addresses_path) as fp:
            line = fp.readline()
            pbar = tqdm(total=batch, position=self.position, desc="Shard " + str(self.index+1) + "/" + str(self.shard))
            while line:
                if (count >= end):
                    break
                ...
                pbar.set_postfix(meta)
                #print(count, '/', adress_count, round(count_effective * 100 / batch, 2),
                #    '%', 'empty', empty, round(empty*100/count_effective, 2), '%')
                try:
                    api = Contract(address=address_path, api_key=self.token)
...
if __name__ == "__main__":
    ...
    cdm = ContractsDownloadManager(token, addresses_path, output_dir, shard, index, skip)
    cdm.download()
